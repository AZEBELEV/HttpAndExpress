...
function processHttpRequest($method, $uri, $headers, $body) {
  ...
  if ($method == "GET" && $uri.startsWith("/sum?nums=")) {
    ...
    if ($uri.startsWith("/sum") && !$uri.startsWith("/sum?nums=")) {
  ...
  if ($method == "POST") {
    ...
    if ($uri != "/api/checkLoginAndPassword" ||
      new Map($headers).get("Content-Type") != "application/x-www-form-urlencoded") {
      statusCode = "400 Bad Request"
    } else {
      if (passwordMap == "not exist") {
        statusCode = "500 - Internal Server Error";
      } else {

        let userPassword = parseInt($body.slice($body.lastIndexOf("=") + 1));
        let userName = $body.match(/(?<=\=)([\s\S]+?)(?=&)/)[0]
        passwordMap.get(userName) == userPassword ?
          body = `<h1 style="color:green">FOUND</h1>` : body = `<h1 style="color:red">NOT FOUND</h1>`
        statusCode = "200 OK";
        statusMessage = body;
      }
    }
...
function getPasswords() {
  ...
  try {
    passwordMap = new Map(fs
      .readFileSync("password.txt", "utf8")
      .split("\n")
      .map(elem => {
        let div = elem.indexOf(":");
        return [elem.slice(0, div), elem.slice(div + 1)]
      }))
  } catch (e) {
    passwordMap = "not exist";
  }
  return passwordMap;
}
let http = parseTcpStringAsHttpRequest(contents);
let passwordMap = getPasswords();
